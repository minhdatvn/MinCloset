...
class BackupService {
  ...
  Future<void> performBackup() async {
    ...
    logger.i("Đã đọc được ${allItems.length} vật phẩm và ${allOutfits.length} trang phục từ CSDL cục bộ.");
    ...
    // TODO: Thêm logic sao lưu cho closets và wear_log nếu cần
    ...
    logger.i("Hoàn tất việc ghi ${updatedItems.length} vật phẩm và ${updatedOutfits.length} trang phục lên Firestore.");
  }
  ...
  Future<void> _deleteAllUserData(String userId) async {
    try {
      ...
      // Xóa tất cả document trong Firestore
      // Lưu ý: Việc xóa sub-collection từ client không được khuyến khích cho lượng dữ liệu lớn,
      // nhưng với quy mô ứng dụng này thì hoàn toàn chấp nhận được.
      final userDocRef = _firestore.collection('users').doc(userId);
      ...
      // Ném ra lỗi để UI có thể bắt và thông báo cho người dùng
